function [ext_positions,feature] = SIFT_new(img, row, colum)
% clc;
% close all;
% clear;
...
kpd=[];
mag2 =padarray(mag,[8,8],'replicate');
oric2 =padarray(oric,[8,8],'replicate');
for x1=1:2:length(kpl)
    k1=kpl(x1);
    j1=kpl(x1+1);
%     if k1 > 7 && j1 > 7 && k1 < m-8 && j1 < n-8
    p2=mag2(k1:k1+15,j1:j1+15);
    q2=oric2(k1:k1+15,j1:j1+15);
%     else
%         continue;
%     end
    kpmagd=[];
...
kx=kpl(1:2:length(kpl));
ky=kpl(2:2:length(kpl));
feature=zeros(128,length(kx));
for ii=1:128:length(kx)
    feature(:,ii)=kpd(ii:ii+127);
end
ext_positions=[kx;ky];
end
% kpd = reshape(kpd, 128, []);
